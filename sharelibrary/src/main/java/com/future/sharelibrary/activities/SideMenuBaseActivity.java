...
public abstract class SideMenuBaseActivity extends AppCompatActivity implements ExpandableListView.OnChildClickListener,ExpandableListView.OnGroupClickListener {

    protected DrawerLayout mDrawerLayout;
    protected Toolbar mToolbar;
    protected LinearLayout appContent;
    ...
    private void initView() {
        ...
        appContent= (LinearLayout) findViewById(R.id.appContentBorder);
        dotsBorder= (LinearLayout) findViewById(R.id.dotsBorder);
        appContentMenu= (LinearLayout) findViewById(R.id.appContentMenu);

        menuList = (ExpandableListView) findViewById(R.id.main_menu_item_list);
        menuList.setAdapter(menuAdapter);

        mainMenuContentList = initData(mainMenuContentList);
        menuAdapter.setData(mainMenuContentList,menuList);
        ...
        mViewPager = (ViewPager)findViewById(R.id.viewpager_forcecast_fg);
    }

    protected View addMenuHeadView(int layoutResId) {
        View head = LayoutInflater.from(this).inflate(layoutResId, null);
        return addMenuHeadView(head);
    }
    ...
    protected View addMenuFooterView(int layoutResId) {
        ...
        View footer = LayoutInflater.from(this).inflate(layoutResId, null);
...
//        fragmentManager.beginTransaction().replace(R.id.main_content_frame, fragment).commitAllowingStateLoss();
//        mangerDrawerLayout();
//    }
    ...
    protected void setBackGround(int colorId){
        mDrawerLayout.getChildAt(0).setBackgroundResource(colorId);
    }

    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
    protected void setBackGround(Drawable drawable){
        appContent.setBackground(drawable);
    }
     ...
     * @param name
     */
    protected void setPageTitle(int icon, CharSequence name, View.OnClickListener onClickListener) {
        ...
        title.setText(name);
        if (icon != 0) {
            title.setCompoundDrawablesWithIntrinsicBounds(getResources().getDrawable(icon), null, null, null);
        }
    }
    ...
    protected void setMaterialLeftView(int openStr, int closeStr) {
        ActionBarDrawerToggle toggle = new ActionBarDrawerToggle(this, mDrawerLayout, mToolbar, openStr, closeStr);
    ...
    protected void setPageRightView(int icon, CharSequence name, View.OnClickListener onClickListener) {
        ...
        if (icon != 0) {
            rightView.setCompoundDrawablesWithIntrinsicBounds(getResources().getDrawable(icon), null, null, null);
        }
    }
    ...
    protected void setPageLeftView(int icon, CharSequence name, View.OnClickListener onClickListener) {
        ...
        if (icon != 0) {
            pageLeftView.setCompoundDrawablesWithIntrinsicBounds(getResources().getDrawable(icon), null, null, null);
        }
    }
    ...
    protected void showSnackbar(View parent, CharSequence content, CharSequence actionName, View.OnClickListener onClickListener) {
        Snackbar.make(parent, content, Snackbar.LENGTH_SHORT).setAction(actionName, onClickListener).show();
    }

    protected void showSnackbar(CharSequence content, CharSequence actionName, View.OnClickListener onClickListener){
        Snackbar.make(appContent,content,Snackbar.LENGTH_SHORT).setAction(actionName, onClickListener).show();
    }
    ...
    public class MenuAdapter extends BaseExpandableListAdapter {
        ...
        public View getGroupView(final int i, boolean b, View convertView, ViewGroup parent) {
            ...
            ViewHolder holder;
            if (convertView == null) {
                convertView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_side_menu, parent, false);
            ...
            switch (type) {
                ...
                case MainMenu.TYPE_HINT:
                    holder.name.setText(menuBean.getName());
                    holder.name.setCompoundDrawablesWithIntrinsicBounds(parent.getContext().getResources().getDrawable(menuBean.getIcon()), null, parent.getContext().getResources().getDrawable(menuBean.getHint()), null);
                    break;

                case MainMenu.TYPE_NORMAL:
                    holder.name.setText(menuBean.getName());
                    holder.name.setCompoundDrawablesWithIntrinsicBounds(parent.getContext().getResources().getDrawable(menuBean.getIcon()), null, null, null);
                    break;
        ...
        public View getChildView(final int i, int i1, boolean b, View convertView, ViewGroup parent) {
            ...
            if (groupFocusItem == i&&childFocusItem==i1) {
                holder.name.setBackgroundColor(parent.getContext().getResources().getColor(R.color.colorBlack));
                holder.name.setTextColor(Color.WHITE);
            } else {
                holder.name.setBackgroundColor(Color.TRANSPARENT);
                holder.name.setTextColor(parent.getContext().getResources().getColor(R.color.colorWhite));
            }
            int type = menuBean.getType();
            switch (type) {
                case MainMenu.TYPE_NO_ICON:
...
//                holder.name.setLayoutParams(params);
            ...
            holder.name.setPadding(FormatUtils.dip2px(parent.getContext(),10),FormatUtils.dip2px(parent.getContext(),5),0,FormatUtils.dip2px(parent.getContext(),5));
            return convertView;
        }
    ...
    public class MainMenu {
        ...
        public List<MainMenu> getChildMenu() {return childMenu;}
    }
