...
public class AreaSelector {
    ...
    public AreaSelector(Context context, ResultHandler resultHandler,String defaultCity) {
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (mode != null)
                    switch (mode.value) {
                        case 1:
                            selectorArea = new String[]{selectorArea[0], selectorArea[1], selectorArea[2]};
                            break;
                        case 2:
                            break;
                    }
    ...
    private void initData() {
        ...
        for (DBUtils.CityData districtData : DBUtils.newInstance(context).readDistrict(selectorArea[1])) {
        ...
        if ((scrollUnits & SCROLLTYPE.COUNTY.value) == SCROLLTYPE.COUNTY.value && TextUtils.isEmpty(selectorArea[3])) {
            selectorArea[3] = "";
            if(county.size()>0){
                selectorArea[3] = county.get(0);
            }
        }
        if ((scrollUnits & SCROLLTYPE.TOWN.value) == SCROLLTYPE.TOWN.value && TextUtils.isEmpty(selectorArea[4])) {
            selectorArea[4] = "";
            if(town.size()>0){
    ...
    private void excuteScroll() {
        ...
        countyView.setCanScroll(county.size() > 1 && (scrollUnits & SCROLLTYPE.COUNTY.value) == SCROLLTYPE.COUNTY.value);
        townView.setCanScroll(town.size() > 1 && (scrollUnits & SCROLLTYPE.TOWN.value) == SCROLLTYPE.TOWN.value);
    }
    ...
    private void districtChange() {
        district.clear();
        for (DBUtils.CityData cityData : DBUtils.newInstance(context).readDistrict(selectorArea[1])) {
    ...
    private void countyChange() {
        if ((scrollUnits & SCROLLTYPE.COUNTY.value) == SCROLLTYPE.COUNTY.value) {
            county.clear();
            for (DBUtils.CityData cityData : DBUtils.newInstance(context).readDistrict(selectorArea[2])) {
    ...
    private void townChange() {
        if ((scrollUnits & SCROLLTYPE.COUNTY.value) == SCROLLTYPE.COUNTY.value) {
            town.clear();
            for (DBUtils.CityData cityData : DBUtils.newInstance(context).readDistrict(selectorArea[3])) {
