...
public class SDCardImageLoader {
    ...
    public SDCardImageLoader(int screenW, int screenH) {
        ...
        // 获取应用程序最大可用内存
        int maxMemory = CommonUtils.getMaxMemory();
    ...
    private Bitmap loadDrawable(final int smallRate, final String filePath,
        ...
        // 如果缓存过就从缓存中取出数据
        if (!TextUtil.isEmpty(filePath) && imageCache.get(filePath) != null) {
            ...
            public void imageLoaded(Bitmap bmp) {
                if (imageView.getTag().equals(filePath)) {
                    if (bmp != null) {
                        ...
                        imageView.setImageResource(R.mipmap.ic_empty_photo);
                    }
                }
            }
        });
    ...
    public void putCache(String filePath, Bitmap bitmap) {
